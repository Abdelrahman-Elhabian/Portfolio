...
function typeWriter(element, text, speed = 100) {
    ...
    function type() {
        if (i < plainText.length) {
            ...
            if (currentText.includes('Abdelrahman Elhabian')) {
                const parts = currentText.split('Abdelrahman Elhabian');
                if (parts.length > 1) {
                    element.innerHTML = parts[0] + '<span class="gradient-text">Abdelrahman Elhabian</span>' + parts[1];
                } else {
                    element.innerHTML = currentText;
                }
            } else if (currentText.length > 8) { // After "Hi, I'm "
                // Check if we're typing the name part
                const nameStart = currentText.indexOf('Abdelrahman');
                if (nameStart !== -1) {
            ...
            element.innerHTML = 'Hi, I\'m <span class="gradient-text">Abdelrahman Elhabian</span>';
        }
    }
